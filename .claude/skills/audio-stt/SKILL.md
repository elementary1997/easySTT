...
## GPU (whisper.cpp via whisper-rs)

- Release/CI: CPU-only. For a **local GPU build** use one Cargo feature: `whisper-cuda` (NVIDIA + CUDA Toolkit), `whisper-vulkan`, or `whisper-metal` (macOS). Example: `cargo tauri build -- --features whisper-cuda` or `npm run tauri:build:gpu`.
- `whisper_init_parameters()` in `local.rs` uses `WhisperContextParameters::default()` (GPU on when the matching whisper-rs feature is enabled). Set `EASYSTT_WHISPER_NO_GPU=1` to force CPU at runtime.
